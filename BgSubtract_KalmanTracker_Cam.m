%% Background Subtraction and Kalman Filter with SORT Tracking Camera Input
%  Code provided and not modified from MATLAB
...
% Motion-Based Multiple Object Tracking> and uses the
% |<docid:driving_ref#bvigrmf-1 multiObjectTracker>| available in Automated
% Driving Toolbox(TM).
%
...
%
% For more information, see <docid:vision_ug#buq9qny-1 Multiple Object Tracking>.
%
% This example is a function, with the main body at the top and helper
% routines in the form of nested functions below. See
% <docid:matlab_prog#f4-39683 Nested Functions> for more details.

function BgSubtract_KalmanTracker_Cam()
% Create objects used for reading video and displaying the results.
videoObjects = setupVideoObjects();
...
%   decrease this value.
% # |DeletionThreshold|: How long a track is maintained before deletion.
%   In this case, since the video has 30 frames per second, a reasonable
%   value is about 0.75 seconds (22 frames).
% # |ConfirmationThreshold|: The parameters controlling track confirmation.
...
frameCount = 0;
while videoObjects.videoPlayer.isOpen()
    % Read a video frame and detect objects in it.
    frameCount = frameCount + 1;                                % Promote frame count
    frame = snapshot(videoObjects.reader);                     % Read frame 
    step(videoObjects.videoPlayer, frame);
...
% Create objects used for reading and displaying the video frames.
    ...
    function videoObjects = setupVideoObjects()
        % Initialize video I/O
        % Create objects for reading a video from a file, drawing the tracked
        % objects in each frame, and playing the video.
        
        videoObjects.reader = webcam('Logitech');
        videoObjects.videoPlayer = vision.VideoPlayer;
        frame = snapshot(videoObjects.reader);
        step(videoObjects.videoPlayer, frame);
    end
